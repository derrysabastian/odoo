...
export class MainMenu extends Component {
    ...
    static components = {};
    static template = "wms_barcode.MainMenu";
    ...
    setup() {
        ...
        onWillStart(async () => {
            const data = await rpc("/wms_barcode/get_main_menu_data");
            ...
            if (this.soundEnable) {
                ...
                this.sounds = {
                    success: new Audio(
                        url(`/wms_barcode/static/src/audio/success.${fileExtension}`)
                    ),
                };
    ...
    removeDemoMessage() {
        const params = {
            ...
            confirm: () => {
                rpc("/wms_barcode/rid_of_message_demo_barcodes"); // Sets action message param false on server
    ...
    async _onBarcodeScanned(barcode) {
        const res = await rpc("/wms_barcode/scan_from_main_menu", { barcode });
        if (res.action) {
    ...
    get demoMessage() {
        ...
        const sheet_s = markup`<a href="/wms_barcode/static/img/barcodes_demo.pdf" target="_blank" aria-label="${demo_link}" title="${demo_link}">`;
        const ops_s = markup`<a href="/wms_barcode/print_inventory_commands?barcode_type=barcode_commands_and_operation_types" target="_blank" aria-label="${barcode_link}" title="${barcode_link}">`;
...
registry.category("actions").add("wms_barcode_main_menu", MainMenu);
