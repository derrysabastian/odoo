...
export default class LazyBarcodeCache {
     ...
     * @returns copy of the record send by the server (fields limited to _get_fields_wms_barcode)
     */
    ...
    async getQuants(product, location_id, params = {}) {
        ...
        if (!quantsByProduct) {
            ...
            const result = await rpc("/wms_barcode/get_quants", { domain });
            if (result) {
    ...
    async _getMissingRecord(barcode, model, filters = {}) {
        ...
        const result = await rpc("/wms_barcode/get_specific_barcode_data", params);
            ...
            if (params.forceUnrestrictedSearch) {
                ...
                if (missingBarcodes.size) {
                    ...
                    const notRestrictedByModelResult = await rpc(
                        "/wms_barcode/get_specific_barcode_data",
                        updatedParams
                    );
