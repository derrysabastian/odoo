...
from odoo.addons.wms_barcode.models.epc_encoder import EpcScheme


class StockBarcodeController(http.Controller):

    @http.route('/wms_barcode/scan_from_main_menu', type='jsonrpc', auth='user')
    ...
    @http.route('/wms_barcode/save_barcode_data', type='jsonrpc', auth='user')
    def save_barcode_data(self, model, res_id, write_field, write_vals):
        ...
        return target_record._get_wms_barcode_data()

    @http.route('/wms_barcode/get_barcode_data', type='jsonrpc', auth='user')
    def get_barcode_data(self, model, res_id):
        ...
        data = target_record._get_wms_barcode_data()
        ...
        mute_sound = request.env['ir.config_parameter'].sudo().get_param('wms_barcode.mute_sound_notifications')
        data['config'] = data.get('config', {})
        data['config']['play_sound'] = bool(not mute_sound or mute_sound == "False")
        data['config']['barcode_separator_regex'] = request.env['ir.config_parameter'].sudo().get_param('wms_barcode.barcode_separator_regex', '.^')
        data['config']['barcode_rfid_batch_time'] = int(request.env['ir.config_parameter'].sudo().get_param('wms_barcode.barcode_rfid_batch_time', 1000))
        delay_between_scan = request.env['ir.config_parameter'].sudo().get_param('wms_barcode.delay_between_scan')
    ...
    @http.route('/wms_barcode/get_main_menu_data', type='jsonrpc', auth='user')
    ...
    @http.route('/wms_barcode/get_specific_barcode_data', type='jsonrpc', auth='user')
    def get_specific_barcode_data(self, **kwargs):
        ...
        for model_name, barcodes in barcodes_by_model.items():
            ...
            fetched_data = self._get_records_fields_wms_barcode(records)
        ...
        if fetch_quant and product_ids:
            ...
            fetched_data = self._get_records_fields_wms_barcode(quants)

            for f_model_name in fetched_data:
                result[f_model_name] = result[f_model_name] + fetched_data[f_model_name]
        return result

    def _get_records_fields_wms_barcode(self, records):
        result = defaultdict(list)
        result[records._name] = records.read(records._get_fields_wms_barcode(), load=False)
        if hasattr(records, '_get_wms_barcode_specific_data'):
            records_data_by_model = records._get_wms_barcode_specific_data()
            for res_model in records_data_by_model:
                result[res_model] += records_data_by_model[res_model]
        return result

    @http.route('/wms_barcode/get_quants', type='jsonrpc', auth='user')
    def get_existing_quant_and_related_data(self, domain):
        quants = request.env['stock.quant'].search(domain)
        return quants.get_wms_barcode_data_records()

    @http.route('/wms_barcode/rid_of_message_demo_barcodes', type='jsonrpc', auth='user')
    def rid_of_message_demo_barcodes(self, **kw):
        ...
        action = request.env.ref('wms_barcode.wms_barcode_action_main_menu')
        action and action.sudo().write({'params': {'message_demo_barcodes': False}})

    @http.route('/wms_barcode/print_inventory_commands', type='http', auth='user')
    ...
    def _try_open_product_location(self, barcode):
        ...
        if result:
            ...
            kanban_view_id = request.env.ref('wms_barcode.stock_quant_barcode_kanban_2').id
            return {
                'action': {
    ...
    def _get_barcode_pdfs(self, barcode_type, domain):
        ...
        if barcode_type == 'barcode_commands_and_operation_types':
            with file_open('wms_barcode/static/img/barcodes_actions.pdf', "rb") as commands_file:
    ...
    @http.route('/wms_barcode/get_epc', type='jsonrpc', auth='user')
    ...
    @http.route('/wms_barcode/get_epc_sgtin', type='jsonrpc', auth='user')
