...
class ResConfigSettings(models.TransientModel):
    ...
    wms_barcode_demo_active = fields.Boolean("Demo Data Active", compute='_compute_wms_barcode_demo_active')
    show_barcode_nomenclature = fields.Boolean(compute='_compute_show_barcode_nomenclature')
    wms_barcode_mute_sound_notifications = fields.Boolean(
        "Mute Barcode application sounds",
        config_parameter="wms_barcode.mute_sound_notifications",
    )
    ...
    barcode_rfid_batch_time = fields.Integer(
        "RFID Timer", config_parameter="wms_barcode.barcode_rfid_batch_time"
    )
    barcode_separator_regex = fields.Char(
        "Multiscan Separator", config_parameter="wms_barcode.barcode_separator_regex",
    ...
    def _compute_show_barcode_nomenclature(self):
        ...
        for rec in self:
            rec.show_barcode_nomenclature = rec.module_wms_barcode and barcode_nomenclature_count > 1

    @api.depends('company_id')
    def _compute_wms_barcode_demo_active(self):
        for rec in self:
            rec.wms_barcode_demo_active = bool(self.env['ir.module.module'].search([('name', '=', 'wms_barcode'), ('demo', '=', True)]))
