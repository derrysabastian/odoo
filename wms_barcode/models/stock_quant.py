...
class StockQuant(models.Model):
    ...
    def barcode_write(self, vals):
        ...
        return self.browse(quant_ids)._get_wms_barcode_data()
    ...
    def _get_wms_barcode_data(self):
        ...
        data = valid_quants.with_context(display_default_code=False, barcode_view=True).get_wms_barcode_data_records()
        if locations:
        ...
        data['line_view_id'] = self.env.ref('wms_barcode.stock_quant_barcode').id
        return data
    ...
    def get_wms_barcode_data_records(self):
        ...
        data = {
            "records": {
                "stock.quant": self.read(self._get_fields_wms_barcode(), load=False),
                ...
                "res.partner": owners.read(owners._get_fields_wms_barcode(), load=False),
    ...
    def _get_wms_barcode_specific_data(self):
        return {
            ...
            'res.partner': self.owner_id.read(self.env['res.partner']._get_fields_wms_barcode(), load=False),
        }
