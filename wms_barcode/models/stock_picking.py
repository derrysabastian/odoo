...
class StockPicking(models.Model):
    ...
    def action_cancel_from_barcode(self):
        self.ensure_one()
        view = self.env.ref('wms_barcode.wms_barcode_cancel_operation_view')
        return {
            'name': _('Cancel this operation?'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'wms_barcode.cancel.operation',
            'views': [(view.id, 'form')],
            'view_id': view.id,
            'target': 'new',
    ...
    def action_open_picking_client_action(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("wms_barcode.wms_barcode_picking_client_action")
    ...
    def _get_wms_barcode_data(self):
        ...
        data = {
            "records": {
                "stock.picking": self.read(self._get_fields_wms_barcode(), load=False),
                "stock.picking.type": self.picking_type_id.read(self.picking_type_id._get_fields_wms_barcode(), load=False),
                "stock.move": self.move_ids.read(self.move_ids._get_fields_wms_barcode(), load=False),
                "stock.move.line": move_lines.read(move_lines._get_fields_wms_barcode(), load=False),
                # `self` can be a record set (e.g.: a picking batch), set only the first partner in the context.
                "product.product": products.with_context(partner_id=self[:1].partner_id.id).read(products._get_fields_wms_barcode(), load=False),
                "res.partner": partners.read(partners._get_fields_wms_barcode(), load=False),
                "stock.location": locations.read(locations._get_fields_wms_barcode(), load=False),
                "stock.package.type": package_types.read(package_types._get_fields_wms_barcode(), False),
                ...
                "product.uom": product_uoms.read(product_uoms._get_fields_wms_barcode(), load=False),
            },
        ...
        data['line_view_id'] = self.env.ref('wms_barcode.stock_move_line_product_selector').id
        data['form_view_id'] = self.env.ref('wms_barcode.stock_picking_barcode').id
        data['scrap_view_id'] = self.env.ref('wms_barcode.scrap_product_selector').id
        data['package_view_id'] = self.env.ref('wms_barcode.stock_quant_barcode_kanban').id
        return data
    ...
    def filter_on_barcode(self, barcode):
        ...
        action = picking_type._get_action('wms_barcode.stock_picking_action_kanban')
