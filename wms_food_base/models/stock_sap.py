...
class FoodStockSap(models.Model):
    _inherit = 'stock.sap'

    # stock.sap has no core Odoo equivalent, so cron_synchronize_sap_stock
    # (wms_base_warehouse) is not "customizing" a native behavior we can jump
    # back to; per the FOOD/FEED design this SAP sync simply must not run
    # for FOOD companies, so we strip their rows (WERKS) out of the payload
